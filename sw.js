...
self.addEventListener('push', (event) => {
  ...
  if (event.data) {
    try {
      ...
      notificationData = {
        ...
        type: payload.type // Extract type (e.g., 'call_incoming')
      };
    } catch (e) {
  ...
  const options = {
    ...
    vibrate: notificationData.type === 'call_incoming' 
    ...
    timestamp: Date.now(),
    // Action buttons
    actions: notificationData.type === 'call_incoming'
      ? [
...
self.addEventListener('notificationclick', (event) => {
  ...
  // Handle dismiss/decline action
  if (event.action === 'dismiss' || event.action === 'decline') {
    console.log('[SW] User dismissed/declined notification');
    return;
  }
  ...
  // If user clicked 'accept', append flag
  if (event.action === 'accept') {
    urlToOpen += (urlToOpen.includes('?') ? '&' : '?') + 'accepted=true';
  }
